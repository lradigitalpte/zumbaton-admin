 ...
 * 4. https://onestepfitness.sg (production default)
 ...
 * 
 * Note: This should return the WEB app URL (onestepfitness.sg), not the admin app URL (admin.onestepfitness.sg)
 */
export function getWebAppUrl(): string {
  ...
  if (!webAppUrl) {
    ...
    if (process.env.NODE_ENV !== 'development') {
      ...
      if (webAppUrl.includes('admin.onestepfitness.sg') || webAppUrl.includes('/admin')) {
        console.warn('[EmailURL] Detected admin URL, overriding to use web app URL: https://onestepfitness.sg')
        webAppUrl = 'https://onestepfitness.sg'
        ...
        console.error('[EmailURL] Please set NEXT_PUBLIC_WEB_APP_URL=https://onestepfitness.sg in Vercel environment')
        webAppUrl = 'https://onestepfitness.sg'
      }
    }
  }

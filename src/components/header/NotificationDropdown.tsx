...
export default function NotificationDropdown({ 'data-onboarding': dataOnboarding }: NotificationDropdownProps = {} as NotificationDropdownProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        {unreadCount > 0 && (
          <span className="absolute right-0 top-0.5 z-10 flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full bg-orange-500 text-white text-[10px] font-semibold">
          ...
          {isLoading ? (
            ...
            notifications.map((notification) => (
              <li key={notification.id}>
                ...
                >
                  <span className={`relative flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${getNotificationColor(notification.type)} text-white`}>
                    {getNotificationIcon(notification.type)}
                    {!notification.readAt && (
                      <span className="absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-orange-500 border-2 border-white dark:border-gray-900"></span>
                    )}
                  </span>

                  <span className="block flex-1 min-w-0">
                    <span className="mb-1.5 block text-sm text-gray-800 dark:text-white/90 line-clamp-2">
